...
def debug_board(tiles, label="BOARD"):
  debug(f"\n--- {label} ---")
...
def move_tiles(window, tiles, clock, direction):
  # debugging code
  debug("\n==============================")
  debug(f"MOVE: {direction.upper()}")
  debug_board(tiles, "START")
  ...
  while updated:
    ...
    for i, tile in enumerate(sorted_tiles):
      # debugging code
      debug(
      f"Tile {tile.value} at ({tile.row},{tile.col}) "
      f"pixel=({tile.x:.1f},{tile.y:.1f})"
      )
      ...
      if not next_tile:
        ...
        debug(
          f"  → merge check with ({next_tile.row},{next_tile.col}) "
          f"value={next_tile.value}"
        )
        if merge_check(tile, next_tile):
          debug("    → not aligned yet, sliding closer")
          tile.move(delta)
        else:
          debug("    → MERGE OCCURS")
          next_tile.value *= 2
        ...
        debug("  → space exists, sliding")
        tile.move(delta)
    
      else:
        debug("  → blocked, no move")
        continue
    ...
    debug_overlaps(tiles)

  #debugging code
  debug_board(tiles, "END")
  debug("==============================\n")
